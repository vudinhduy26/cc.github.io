...
function drawTextOnCanvas(text, fontSize, color) {
...
// Function to create fireworks in the shape of text
function createFireworksText(text, fontSize, color, x, y) {
  const textCanvas = drawTextOnCanvas(text, fontSize, color);
  const textCtx = textCanvas.getContext('2d');
  const imageData = textCtx.getImageData(0, 0, textCanvas.width, textCanvas.height).data;

  for (let py = 0; py < textCanvas.height; py++) {
    for (let px = 0; px < textCanvas.width; px++) {
      const index = (py * textCanvas.width + px) * 4;
      const alpha = imageData[index + 3];
      if (alpha > 128) { // If pixel is not transparent
        createParticle(x + px, y + py, color);
      }
    }
  }
}
...
function createParticle(x, y, color) {
  ...
  // For example, you can create particles and animate them
  console.log(`Particle at (${x}, ${y}) with color ${color}`);
}
...
const ttShell = (size = 1) => {
  ...
  return {
    ...
    color: COLOR.Gold,
    pistil: Math.random() < 0.5,
    pistilColor: makePistilColor(COLOR.Red),
    tt: true
  };
  // return Object.assign({}, defaultShell, config);
};

const ggShell = (size = 1) => {
  const color = randomColor();
  return {
    color,
    shellSize: size,
...
const duyShell = (size = 1) => {
  const color = randomColor();
  const pistil = Math.random() < 0.70;
  return {
    shellSize: size,
    heart: true,
    color,
...
function randomShellName() {
  return Math.random() < 0.5 ? 'Diep' : shellNames[(Math.random() * (shellNames.length - 1) + 1) | 0];
}
